...
export default async function AdminCollectiveRegistrationsPage() {
  await requireAdmin();
  const supabase = await createSupabaseServerClient();

  const [registrationsRes, teamsRes] = await Promise.all([
    supabase
      .from("v_admin_activity_registrations_collective")
      .select("*")
  ...
  const tabs = getActivitiesByCategory("collective_sport").map((activity) => ({
    slug: activity.slug,
  ...
  return (
    <div className="space-y-8">
      <AdminPageHeader
        ...
        title="Collective Sports Registrations"
        description="Review and manage collective sport registrations by activity tabs."
        stats={[
